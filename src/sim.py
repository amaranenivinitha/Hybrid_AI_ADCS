...
import pandas as pd  # ✅ Added for logging simulation data
import os
...
def run_single():
    ...
    # ✅ Added lists for saving full results
    quat_log = []
    pd_error_log = []
    ai_error_log = []
    tau_pd_log = []
    ...
    for t in tvec:
        ...
        quat_log.append(state[0:4])
        pd_error_log.append(np.linalg.norm(q_err) * 57.3)  # PD error deg
        ai_error_log.append(err_deg)  # Actual AI hybrid error deg
        tau_pd_log.append(np.linalg.norm(tau_pd))
        tau_ai_log.append(np.linalg.norm(tau_ai))
    ...
    plt.title("Satellite Attitude Control (PD + AI Hybrid)")
    plt.grid(True)
    ...
    # ✅ Save full data to CSV for animations
    ...
    df = pd.DataFrame({
        ...
        "q2": quat_log[:,2],
        "q3": quat_log[:,3],
        "pd_error_deg": pd_error_log,
        "ai_error_deg": ai_error_log,
        "tau_pd": tau_pd_log,
        "tau_ai": tau_ai_log
    })
    df.to_csv("results/simulation_quat_log.csv", index=False)
    print("✅ Simulation data saved to results/simulation_quat_log.csv")
